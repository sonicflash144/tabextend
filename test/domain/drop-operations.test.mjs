...
test('drops tabs between columns using the pre-removal drop index', () => {
    ...
    assert.deepEqual(
        result.columns[0].items.map(item => (item.type === 'tab' ? item.tabId : item.id)),
        ['two', 'group-a']
    );
    assert.deepEqual(
        result.columns[1].items.map(item => item.tabId),
        ['one', 'four']
    );
...
test('reorders tabs within a group and removes empty source groups', () => {
    ...
    assert.equal(
        emptied.columns[0].items.some(item => item.id === 'group-a'),
        false
    );
});
...
test('moves a group intact to a new column', () => {
    ...
    assert.equal(
        result.columns[0].items.some(item => item.id === 'group-a'),
        false
    );
